...
For each staged temp file, apply the same one-click capture rule defined below, same parsing, same categorisation and routing, same deduplication by stable ID and content hash against existing vault notes. For daily-agenda/notes.md and project notes.md, treat each new or changed log entry as an immutable source item identified by GitHub path + source commit/blob identity + content hash; export only entries not already represented in the vault/checkpoint. Never delete or rewrite daily-agenda or project history merely because it was exported.
