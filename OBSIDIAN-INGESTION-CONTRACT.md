...
## Review state for refined captures — review by exception — 2026-09-23

`BOB-CAPTURE-INTELLIGENCE.md` is authoritative for capture detection, refinement, inference, review state, and when Benson must be asked.

A refined Bob capture must carry a review state:
- `pending_user`
- `approved`

Default to `approved` for routine high-confidence captures whose meaning, canonical owner, scope, identity, and date/chronology are sufficiently supported. Preserve the original raw wording and raw content hash plus the refined representation and its stable identity/hash when implemented.

Use `pending_user` only when a material unresolved question could change routing, ownership, scope, identity, chronology, task meaning, or creation/merging of a canonical entity. Local Codex must not route/write the affected `pending_user` capture until it is resolved, but must continue processing unrelated approved captures.

Automatically detected durable conversation content may be staged/routed when high-confidence and low-risk under the canonical capture rules. Do not silently create new projects/deals/people or make consequential assumptions.

Review state is independent of PA/PM classification and Personal/Work scope.

When processing the queue, skip:
1. `classification: pending` / legacy `pending-classification:`;
2. any capture whose `review_status` is `pending_user`.

Do not treat a GitHub pending-review entry as an Obsidian delivery. Benson may request review of any capture or batch at any time.
